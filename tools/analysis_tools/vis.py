...
def lidar2img(points_lidar, camrera_info):
    ...
    points_camera = points_camera / np.abs(points_camera[:, 2:3])
...
def main():
    ...
    for cnt, infos in enumerate(
        ...
        if len(pred_boxes) == 0:
            ...
            pred_boxes = np.array(pred_boxes, dtype=np.float32)     # (N_pred, 7)
            boxes = LB(pred_boxes, origin=(0.5, 0.5, 0.5))
            corners_global = boxes.corners.numpy().reshape(-1, 3)   # (N_pred*8, 3)   3:(x, y, z)
            corners_global = np.concatenate(
                [corners_global,
                 np.ones([corners_global.shape[0], 1])],
                axis=1)     # (N_pred*8, 4)  4:(x, y, z, 1)
            l2g = get_lidar2global(infos)
            # (N_pred*8, 4) @ (4, 4) --> (N_pred*8, 4)
            corners_lidar = corners_global @ np.linalg.inv(l2g).T
            corners_lidar = corners_lidar[:, :3]    # (N_pred*8, 3)

        pred_flag = np.ones((corners_lidar.shape[0] // 8, ), dtype=np.bool)     # (N_pred, )
        scores = [
        ...
        if args.draw_gt:
            ...
            # corners_lidar_gt = np.concatenate(
            #     [corners_lidar_gt,
            #      np.ones([corners_lidar_gt.shape[0], 1])],
            #     axis=1)     # (N_gt, 4)  4:(x, y, z, 1)
            # lidar2ego = np.eye(4, dtype=np.float32)
            # lidar2ego[:3, :3] = Quaternion(infos['lidar2ego_rotation']).rotation_matrix
            # lidar2ego[:3, 3] = infos['lidar2ego_translation']
            # corners_lidar_gt = corners_lidar_gt @ np.linalg.inv(lidar2ego).T
            # corners_lidar_gt = corners_lidar_gt[:, :3]    # (N_pred*8, 3)

            corners_lidar = np.concatenate([corners_lidar, corners_lidar_gt],
                                           axis=0)      # (N_pred*8+N_gt*8, 3)
        ...
        scores = np.array(scores, dtype=np.float32)     # (N_pred+N_gt, )
        ...
        for i, view in enumerate(views):
            ...
            # draw instances
            # (N_pred*8+N_gt*8, 2), (N_pred*8+N_gt*8, )
            corners_img, valid_depth = lidar2img(corners_lidar, infos['cams'][view])
            valid = np.logical_and(
                valid_depth,
                check_point_in_img(corners_img, img.shape[0], img.shape[1]))        # (N_pred*8+N_gt*8, )
            valid = valid.reshape(-1, 8)    # (N_pred+N_gt, 8)
            corners_img = corners_img.reshape(-1, 8, 2).astype(np.int)      # (N_pred+N_gt, 8, 2)
            for aid in range(valid.shape[0]):
                ...
                for index in draw_boxes_indexes_img_view:
                    if (valid[aid, index[0]] or valid[aid, index[1]]):
                        cv2.line(
                            img,
                            tuple(corners_img[aid, index[0]]),
                            tuple(corners_img[aid, index[1]]),
        ...
        # draw instances
        # (N_pred*8+N_gt*8, 3) --> (N_pred+N_gt, 8, 3)
        corners_lidar = corners_lidar.reshape(-1, 8, 3)
        corners_lidar[:, :, 1] = -corners_lidar[:, :, 1]
        bottom_corners_bev = corners_lidar[:, [0, 3, 7, 4], :2]     # (N_pred+N_gt, 4, 2)
        bottom_corners_bev = \
            (bottom_corners_bev + show_range) / show_range / 2.0 * canva_size   # (N_pred+N_gt, 4, 2)
        bottom_corners_bev = np.round(bottom_corners_bev).astype(np.int32)      # (N_pred+N_gt, 4, 2)

        center_bev = corners_lidar[:, [0, 3, 7, 4], :2].mean(axis=1)    # (N_pred+N_gt, 2)
        head_bev = corners_lidar[:, [0, 4], :2].mean(axis=1)            # (N_pred+N_gt, 2)
        center_canvas = \
            (center_bev + show_range) / show_range / 2.0 * canva_size   # (N_pred+N_gt, 2)
        center_canvas = center_canvas.astype(np.int32)                  # (N_pred+N_gt, 2)
        head_canvas = (head_bev + show_range) / show_range / 2.0 * canva_size   # (N_pred+N_gt, 2)
        ...
        if args.format == 'image':
            ...
            cv2.imwrite(os.path.join(vis_dir, '%05d.jpg' % cnt), img)
