...
from projects.mmdet3d_plugin.core.bbox.structures import Box3DMode, LiDARInstance3DBoxes, points_cam2img
import mmcv
...
class Waymo2KITTI(object):
    ...
    def __init__(self,
        ...
        self.filter_empty_3dboxes = False
        ...
        if 'testing_3d_camera_only_detection' not in self.load_dir:
            mmcv.mkdir_or_exist(self.point_cloud_save_dir)
        for i in range(5):
            mmcv.mkdir_or_exist(f'{self.image_save_dir}{str(i)}')

    def convert(self):
        """Convert action."""
        print(f'Start converting {self.split} dataset')
        if self.workers == 0:
            data_infos = mmcv.track_progress(self.convert_one, range(len(self)))
        else:
        ...
        print(f'Saving {self.split} dataset infos into {filenames}')
        mmcv.dump(waymo_infos, filenames)
    ...
    def gather_cam_instance_info(self, instances: dict, images: dict):
        ...
        for cam_type in self.camera_types:
            ...
            for instance in instances:
                ...
                cam_instance['bbox_3d'] = gt_bboxes_3d.tensor.numpy().squeeze(
    ...
    def merge_trainval_infos(self):
        ...
        train_infos = mmcv.load(train_infos_path)
        val_infos = mmcv.load(val_infos_path)
        trainval_infos = dict(
