...
from ..model_utils import DepthNet, DepthNetSOLOFusion
...
class PolarLSSViewTransformer(BaseModule):
    ...
    def create_grid_infos(self, azimuth, radius, z, **kwargs):
        """Generate the grid information including the lower bound, interval,
        and size.
        """
        self.grid_lower_bound = torch.Tensor([cfg[0] for cfg in [azimuth, radius, z]])     # (min_r, min_d, min_z)
        self.grid_interval = torch.Tensor([cfg[2] for cfg in [azimuth, radius, z]])        # (dr, dd, dz)
        self.grid_size = torch.Tensor([(cfg[1] - cfg[0]) / cfg[2]
                                       for cfg in [azimuth, radius, z]])     # (Dr, Dd, Dz)
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ...
        if ranks_feat is None:
            ...
            ]).to(feat)     # (B, C, Dz, Dd, Dr)
            dummy = torch.cat(dummy.unbind(dim=2), 1)   # (B, C*Dz, Dd, Dr)
            return dummy
    ...
    def voxel_pooling_prepare_v2(self, coor):
        ...
        Returns:
            tuple[torch.tensor]:
                ...
                interval_starts: (N_pillar, )  该pillar的起始位置  (N_pillar, )
                interval_lengths: (N_pillar, )  该pillar包含points的数量 (N_pillar, )
        """
        ...
        # ((B, N, D, fH, fW, 3) - (3, )) / (3, ) --> (B, N, D, fH, fW, 3)   3:(x, y, z)  polar grid坐标.
        coor = ((coor - self.grid_lower_bound.to(coor)) /
                self.grid_interval.to(coor)).long()
        coor = coor.view(num_points, 3)      # (B, N, D, fH, fW, 3) --> (B*N*D*fH*fW, 3)
        ...
        kept[1:] = ranks_bev[1:] != ranks_bev[:-1]  # 边界点=1, 其余为0（pillar id发生变化）
        interval_starts = torch.where(kept)[0].int()    # 该pillar的起始位置  (N_pillar, )
        if len(interval_starts) == 0:
            return None, None, None, None, None
        interval_lengths = torch.zeros_like(interval_starts)
        interval_lengths[:-1] = interval_starts[1:] - interval_starts[:-1]
        interval_lengths[-1] = ranks_bev.shape[0] - interval_starts[-1]     # pillar包含points的数量  (N_pillar, )
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        def cart2polar(coord):
            """
            Args:
                coord: (B, N, D, H, W, 3)   3: (x, y, z)
            Returns:
                polar_coord: (B, N, D, H, W, 3)   3: (r, d, z)
            """
            phi = torch.atan2(coord[..., 1], coord[..., 0])  # (-pi, pi)
            rho = torch.sqrt(coord[..., 0] ** 2 + coord[..., 1] ** 2)
            return torch.stack((phi, rho, coord[..., 2]), dim=-1)
        ...
        if self.accelerate:
            ...
            coor = self.get_ego_coor(*input[1:7])   # (B, N, D, fH, fW, 3)
            coor = cart2polar(coor)     # (B, N, D, fH, fW, 3)
...
class PolarLSSViewTransformerBEVDepth(PolarLSSViewTransformer):
    def __init__(self, mid_channels=256, loss_depth_weight=3.0, depthnet_cfg=dict(), **kwargs):
        super(PolarLSSViewTransformerBEVDepth, self).__init__(**kwargs)
    ...
    def get_mlp_input(self, sensor2ego, ego2global, intrin, post_rot, post_tran, bda):
        ...
        mlp_input = torch.stack([
            ...
            intrin[:, :, 0, 2],     # cx
            intrin[:, :, 1, 2],     # cy
            post_rot[:, :, 0, 0],   # post_rot 和 post_tran的有效参数.
            post_rot[:, :, 0, 1],
            post_tran[:, :, 0],
            post_rot[:, :, 1, 0],
            post_rot[:, :, 1, 1],
...
@NECKS.register_module()
class PolarLSSViewTransformerSOLOFusion(PolarLSSViewTransformerBEVDepth):
    def __init__(self,
                 ...
                 stereo_eps=1e-5,
                 **kwargs):
        super(PolarLSSViewTransformerSOLOFusion, self).__init__(**kwargs)
        ...
        self.depth_net = DepthNetSOLOFusion(
            in_channels=self.in_channels,
            mid_channels=self.mid_channels,
            context_channels=self.out_channels,
            depth_channels=self.D,
            depth_bound=self.grid_config['depth'],
            stereo_sampling_num=self.stereo_sampling_num,
            stereo_group_num=self.stereo_group_num,
            stereo_gauss_bin_stdev=self.stereo_gauss_bin_stdev,
            stereo_eps=self.stereo_eps,
            cam_channel=33,
            **kwargs['depthnet_cfg']
        )
    ...
    def get_mlp_input(self, sensor2ego, ego2global, intrin, post_rot, post_tran, bda):
        ...
        Args:
            ...
            bda: (B, 3, 3)
        Returns:
            mlp_input: (B, N_views, 15)
        """
        B, N, _, _ = sensor2ego.shape

        bda_matrix = sensor2ego.new_zeros((B, 4, 4))
        bda_matrix[:, :3, :3] = bda
        bda_matrix[:, 3, 3] = 1.0
        bda_matrix = bda_matrix.view(B, 1, 4, 4).repeat(1, N, 1, 1)
        sensor2augegos = bda_matrix.matmul(sensor2ego)
        rots = sensor2augegos[..., :3, :3]
        trans = sensor2augegos[..., :3, -1]
    ...
    def forward(self, input, stereo_metas=None):
        ...
        Returns:
            ...
            depth: (B*N, D, fH, fW)
        """
        (x, sensor2egos, ego2globals, intrins, post_rots, post_trans, bda,
         mlp_input) = input[:8]
