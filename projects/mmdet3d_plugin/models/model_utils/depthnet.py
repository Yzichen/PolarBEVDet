...
class DepthNet(nn.Module):
    ...
    def gen_grid(self, metas, B, N, D, H, W, hi, wi):
        ...
        Args:
            metas: dict{
                ...
                cv_feat_list: [feat_prev_iv, stereo_feat]   # 上一帧和当前帧对应的stereo feature.  (B*N_views, C_stereo, fH_stereo, fW_stereo)
            }
            B: batchsize
            N: N_views
            D: D
    ...
    def calculate_cost_volumn(self, metas):
        ...
        for fid in range(curr.shape[1] // group_size):
            ...
            # 用对应feature之间的差异和作为matching cost, cost越小，说明匹配的越准确, 这比用特征之间的点积效果更好吗？
        ...
        if not self.bias == 0:
            # 在某一深度假设下，一像素找不到对应点，我们增加他的matching cost.
    ...
    def forward(self, x, mlp_input, stereo_metas=None):
        ...
        if not stereo_metas is None:
            if stereo_metas['cv_feat_list'][0] is None:
                ...
                with torch.no_grad():
                    # 只利用了每个相机的时序信息来构成stereo, 但是并没有利用跨相机的信息来构成stereo.
                    # https://github.com/HuangJunJie2017/BEVDet/issues/278
